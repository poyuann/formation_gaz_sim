...
{
    //  ROS_initialize  //
    ros::init(argc, argv, "foryolo");
    ros::NodeHandle nh;
    //subscribe
    ros::Subscriber pose_sub = nh.subscribe<geometry_msgs::PoseStamped>("typhoon_h4801/mavros/local_position/pose",10,&pose_cb);
    ros::Subscriber vel_sub = nh.subscribe<geometry_msgs::TwistStamped>("typhoon_h4801/mavros/local_position/twist",10,&vel_cb);
    // publisher
    ros::Publisher desired_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("typhoon_h4801/mavros/setpoint_position/local", 10);
    ros::Publisher vel_cmd_pub = nh.advertise<geometry_msgs::TwistStamped>("typhoon_h4801/mavros/setpoint_velocity/cmd_vel", 10);
    
    ros::ServiceClient arming_client = nh.serviceClient<mavros_msgs::CommandBool>("typhoon_h4801/mavros/cmd/arming");
    ros::ServiceClient set_mode_client = nh.serviceClient<mavros_msgs::SetMode>("typhoon_h4801/mavros/set_mode");
    ros::ServiceClient land_client = nh.serviceClient<mavros_msgs::CommandTOL>("typhoon_h4801/mavros/cmd/land");
    ...
    while (ros::ok()) 
    {
        ...
        {   
            ...
            desired_pose.pose.position.x = current_x + 3*cos(trajectory_time);
            desired_pose.pose.position.y = current_y + 3*sin(trajectory_time);
            desired_pose.pose.position.z = 12;
